...
// ─── PARSEAR BLOQUE ```products``` DE LA RESPUESTA DE N8N ─────────────────────
...
// ─── COLORES Y ETIQUETAS DE STOCK (datos vienen de n8n, ya son reales) ────────
...
function stockLabel(stock?: number, statusLabel?: string): string {
  if (statusLabel && statusLabel !== 'desconocido') return statusLabel;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <circle cx="82" cy="44" r="9" fill="#00C2E0" />
        <circle cx="118" cy="44" r="9" fill="#00C2E0" />
        <circle cx="85" cy="41" r="3" fill="white" fillOpacity="0.85" />
        <circle cx="121" cy="41" r="3" fill="white" fillOpacity="0.85" />
        <path
          d="M90 59 Q100 65 110 59"
          fill="none"
          stroke="#00D1FF"
          strokeWidth="2.5"
          strokeLinecap="round"
...
export default function Chatbot() {
  ...
  // Añadir al carrito de PrestaShop usando la URL real del producto
  const handleAddToCart = useCallback(
    (card: ProductCard, buyNow: boolean) => {
      // Extraer id_product de la URL real que viene de n8n
      const urlMatch = card.url.match(/id_product=(\d+)/);
      if (!urlMatch) {
        window.open(card.url || PS_HOME, '_blank', 'noopener,noreferrer');
        return;
      }
      const psId = parseInt(urlMatch[1], 10);
      ...
      window.open(
        buildAddToCartUrl(psId, qty),
        '_blank',
        'noopener,noreferrer'
      );
      if (buyNow) {
        setTimeout(
          () => window.open(PS_CART_PAGE, '_blank', 'noopener,noreferrer'),
          400
        );
      }
    },
  ...
  const send = useCallback(
    async (
      text: string,
      imageData?: { base64: string; dataUrl: string; mimeType: string } | null
    ) => {
      const t = text.trim();
      ...
      try {
        const userQuery =
          t ||
          'Identifica este artículo de la imagen, dame referencia exacta y botón de compra.';
        ...
        // Una sola llamada: n8n devuelve todo (precio real + stock real de PS)
        ...
        const reply: string =
          ...
          data.output ??
          data.text ??
          'Lo siento, no pude procesar la solicitud. Por favor, inténtalo de nuevo.';
        ...
        setMessages((p) => [
          ...p,
          {
            ...
            role: 'bot',
            content:
              'Error de conexión. Por favor, inténtalo de nuevo en unos segundos.',
            ts: new Date(),
          },
        ]);
      } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(160deg, #07101E 0%, #0C1D38 100%)',
            ...
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id}>
                        ...
                        <div
                          ...
                          style={
                            msg.role === 'user'
                              ? {
                                  background:
                                    'linear-gradient(135deg, #0047C8, #0078B8)',
                                  color: 'white',
                                  boxShadow:
                                    '0 4px 16px rgba(0,80,200,0.28)',
                                }
                              : {
                                  background: '#101D30',
                                  color: '#CDD6E3',
                  ...
                  {/* Tarjetas de producto — datos 100% de n8n/PrestaShop */}
                  {cards.length > 0 && (
                    <div className="mt-2 ml-9 flex flex-col gap-3">
                      {cards.map((card, ci) => {
                        const qty = getQty(card.ref);
                        const inCart = cart.find(
                          (i) => i.ref === card.ref.toUpperCase()
                        );
                        const stockNum =
                          typeof card.stock === 'number'
                            ...
                            : undefined;
                        const hasRealUrl =
                          card.url && card.url.includes('id_product=');
                        const canBuy =
                          hasRealUrl && stockNum !== 0;
                        const priceNum =
                          typeof card.priceRaw === 'number'
                            ? card.priceRaw
                            : typeof card.price === 'number'
                              ...
                              <div className="flex items-start justify-between gap-2 mb-1">
                                ...
                                <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                                  <span
                                    ...
                                    style={{
                                      background: stockColor(stockNum),
                                    }}
                                  />
                                  <span
                                    className="text-[10px] font-medium"
                                    style={{ color: stockColor(stockNum) }}
                                  >
                                    {stockLabel(stockNum, card.stockStatus)}
                                  </span>
                                </div>
                              </div>

                              {/* Precio real de PrestaShop */}
                              <div className="flex items-baseline gap-1.5 mb-2">
                                <span
                                  className="text-base font-extrabold"
                                  style={{
                                    color: isLive ? '#00C2E0' : '#94A3B8',
                                  }}
                                >
                                  {priceDisplay}
                                </span>
                                ...
                                {priceNum !== undefined && qty > 1 && (
                                  <span
                                    className="text-[10px] font-semibold"
                                    style={{ color: '#34D399' }}
                                  >
                                    · {(priceNum * qty).toFixed(2)} € total
                                  </span>
                                )}
                              </div>

                              {/* Selector cantidad — solo si hay URL real y hay stock */}
                              {canBuy && (
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <div
                                    ...
                                    style={{
                                      border:
                                        '1px solid rgba(0,150,220,0.22)',
                                    }}
                                  >
                                    <button
                                      onClick={() =>
                                        setQtyFor(card.ref, qty - 1)
                                      }
            ...
            >
              <div className="min-w-0">
                <p
                  ...
                  style={{ color: '#34D399' }}
                >
                  🛒 {cartTotalItems} artículo
                  {cartTotalItems !== 1 ? 's' : ''} en tu carrito
                </p>
                {cartTotalPrice > 0 && (
                  <p
                    className="text-[10px]"
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                boxShadow: pendingImage
                  ? '0 4px 14px rgba(0,150,220,0.4)'
                  : 'none',
              }}
            >
              <svg
                ...
                fill="none"
                stroke={
                  pendingImage ? 'white' : 'rgba(148,163,184,0.85)'
                }
                strokeWidth={1.8}
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '14px',
                background:
                  'linear-gradient(135deg, #0047C8, #0092C2)',
                boxShadow:
      ...
      <div className="relative flex flex-col items-center">
        ...
        <div
          className="mt-2 text-[9px] font-medium tracking-wide"
          style={{ color: 'rgba(71,85,105,0.9)' }}
        >
          powered by{' '}
          <a
